...
func (c *SaslConfig) ConfigureSarama(config *sarama.Config) {
	...
	default:
		...
		panic(fmt.Sprintf("not valid SASL mechanism '%v', only supported with PLAIN|SCRAM-SHA-512|SCRAM-SHA-256|AWS-IAM", c.SaslMechanism))
	}
}
...
func (c *SaslConfig) Validate() error {
	...
	default:
		return fmt.Errorf("not valid SASL mechanism '%v', only supported with PLAIN|SCRAM-SHA-512|SCRAM-SHA-256|AWS-IAM", c.SaslMechanism)
	}
	return nil
}
